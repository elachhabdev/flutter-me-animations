...
class ItemState extends State<Item> with SingleTickerProviderStateMixin {
  ...
  int get crossAxis => widget.crossAxis;
  ...
  Color get color => widget.item.color;
  ...
  void updateForGap(List<int> currentIndexes, Size gapExtent, bool animate) {
    final Offset newPosition;

    int currentIndex = currentIndexes.indexOf(index);

    int rowIndex = (index / crossAxis).floor();
    int colIndex = index % crossAxis;

    int rowCurrentIndex = (currentIndex / crossAxis).floor();
    int colCurrentIndex = currentIndex % crossAxis;

    newPosition = Offset((colCurrentIndex - colIndex) * gapExtent.width,
        (rowCurrentIndex - rowIndex) * gapExtent.height);
  ...
  void initState() {
    super.initState();
    listState = MyReordonableGridState.of(context);
